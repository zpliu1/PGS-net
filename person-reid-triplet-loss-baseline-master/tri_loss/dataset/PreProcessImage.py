...
class PreProcessIm(object):
  ...
  def pre_process_im(self, im):
    ...
    im = Cutout(probability=0.2, size=64, mean=[0.0, 0.0, 0.0])(im)
    im = RandomErasing(probability=0.3, mean=[0.0, 0.0, 0.0])(im)
    return im, mirrored
  
  def test_pre_process_im(self, im):
    """Pre-process image.
    `im` is a numpy array with shape [H, W, 3], e.g. the result of
    matplotlib.pyplot.imread(some_im_path), or
    numpy.asarray(PIL.Image.open(some_im_path))."""
    ...
    if self.batch_dims == 'NCHW':
      im = im.transpose(2, 0, 1)
    #im = Cutout(probability=0.5, size=64, mean=[0.0, 0.0, 0.0])(im)
